...
function CollegesDirectoryInner() {
  ...
  function surpriseMe() {
    ...
    const withLinks = colleges.filter(isLive);
  ...
  const withLinks = colleges.filter(isLive).length;
  const liveShown = shownColleges.filter(isLive);
  const moreShown = shownColleges.filter((c) => !isLive(c));
  const flag = COUNTRY_FLAGS[country] || "🌍";
  const countryTotal = countryCounts[country] ?? 0;
  const countryWithLinks = colleges.filter((c) => (c.country || "") === country && isLive(c)).length;
    ...
    <div className="relative">
      ...
      <div className="relative z-10 space-y-6">
        ...
        <p className="text-sm text-ss-muted">
          Showing <strong className="text-ss-text">{liveShown.length}</strong> with live careers links{moreShown.length > 0 ? ` (+${moreShown.length} coming soon)` : ""} · {shortlistOnly ? "shortlisted colleges" : `colleges in ${country}`}.
        </p>
